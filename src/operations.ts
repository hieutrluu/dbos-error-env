...
import { HandlerContext, TransactionContext, Transaction, GetApi, ArgSource, ArgSources, parseConfigFile, Communicator, CommunicatorContext } from '@dbos-inc/dbos-sdk';
...
export class Hello {
  ...
  static async helloTransaction(ctxt: TransactionContext<Knex>, @ArgSource(ArgSources.URL) user: string) {
    ...
    const dbos_greetings = JSON.stringify(dbosConfig);
    const main = Effect.sleep(1000).pipe(
      // Effect.succeed('Hello, World!'),
      Effect.andThen(Console.log('Hello, World!'))
    );
    const tmp = await Effect.runPromise(main);
    return Hello.makeHTML(greeting + dbos_greetings);
  }
  ...
  static async readme(_ctxt: HandlerContext) {
    ...
    const tmp = Effect.runSync(Effect.succeed(message));
